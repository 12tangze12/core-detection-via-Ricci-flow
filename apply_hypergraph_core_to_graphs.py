...
def extract_core_subgraph_from_largest_component(G_original, G_after_surgery, weight="weight", remove_ratio=0.8):
    """
    手术后提取最大连通分支中的节点，在原图中诱导出核心子图。
    G_original: 原始图（完整）
    G_after_surgery: Ricci流演化+手术后的图（即已经删除部分高权边）
    """
    # 1. 删除高权边
    all_edges = list(G_after_surgery.edges(data=True))
    ...
    G_after_surgery.remove_edges_from(edges_to_remove)


    # 2. 找最大连通分支
    components = list(nx.connected_components(G_after_surgery))
    print(f"手术后剩余连通分量数：{len(components)}")
    largest_component_nodes = max(components, key=len)
    print(f"最大连通分支节点数：{len(largest_component_nodes)}")
    # ✅ 直接在手术后的图中构造最大分支对应的子图并统计边数
    largest_component_subgraph = G_after_surgery.subgraph(largest_component_nodes)
    print(f"最大连通分支边数：{largest_component_subgraph.number_of_edges()}")

    # 3. 在原始图中诱导核心子图
    core_subgraph = G_original.subgraph(largest_component_nodes).copy()
    print(f"在原图中诱导的核心子图节点数：{core_subgraph.number_of_nodes()}，边数：{core_subgraph.number_of_edges()}")
...
def run_ricci_flow_surgery(G_original, alpha=0.1, iterations=40, surgery_every=2, delete_ratio=0.08):
    ...
    # ✅ 提取最大连通分支节点，在原图中诱导核心子图
    core_subgraph = extract_core_subgraph_from_largest_component(G_original, ricci.G)


    print(f"\n最终核心子图：节点数 = {core_subgraph.number_of_nodes()}，边数 = {core_subgraph.number_of_edges()}")
    return core_subgraph
...
# 2. Ricci流 + 删除高权边提取核心图
core_subgraph = run_ricci_flow_surgery(
    G,
    alpha=0.1,
    iterations=40,
    surgery_every=2,
...
# 3. 计算核心相关指标
r_d, runconnected, r_k, r_dist_stretch, r_core_k = compute_core_graph_metrics(G, core_subgraph)

# 4. 打印指标结果
print_core_metrics(r_d, runconnected, r_k, r_dist_stretch, r_core_k)
